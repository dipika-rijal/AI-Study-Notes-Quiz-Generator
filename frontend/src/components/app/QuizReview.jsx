...
export default function QuizReview({ quiz, answers, score, totalQuestions, attemptId, isSaved, onSave, onUnsave, onRetry, onClose }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-3xl border border-purple-100 dark:border-[#424242] bg-white dark:bg-[#171717] p-6 text-center shadow-sm dark:shadow-none">
        <h2 className="text-2xl font-black text-[#6757ff] dark:text-[#10a37f]">Quiz Review</h2>
        <p className="mt-4 text-lg font-black text-[#15132b] dark:text-[#ececec]">
          Score: {score}/{totalQuestions}
        </p>
        <p className="mt-2 text-lg font-black text-orange-500 dark:text-[#10a37f]">
          Accuracy: {accuracy}%
        </p>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3 text-left">
          <div className="rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 p-4 border border-emerald-100 dark:border-emerald-800/50">
            <p className="text-xs font-black uppercase text-emerald-700 dark:text-emerald-400 mb-2">Strong Areas</p>
            <ul className="space-y-1 text-xs font-semibold text-[#15132b] dark:text-[#ececec]">
              {(areas.strong.length ? areas.strong : ["Keep practicing"]).map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl bg-red-50 dark:bg-red-900/20 p-4 border border-red-100 dark:border-red-900/50">
            <p className="text-xs font-black uppercase text-red-700 dark:text-red-400 mb-2">Weak Areas</p>
            <ul className="space-y-1 text-xs font-semibold text-[#15132b] dark:text-[#ececec]">
          ...
          <div className="rounded-2xl bg-purple-50 dark:bg-[#2f2f2f] p-4 border border-purple-100 dark:border-[#424242]">
            <p className="text-xs font-black uppercase text-[#6757ff] dark:text-[#10a37f] mb-2">Recommended</p>
            <ul className="space-y-1 text-xs font-semibold text-[#15132b] dark:text-[#ececec]">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-4 border-t border-purple-50 dark:border-[#424242] pt-6">
          {onRetry && (
            <button
              onClick={onRetry}
              className="rounded-2xl border border-orange-100 dark:border-[#424242] bg-white dark:bg-[#171717] px-5 py-2.5 text-sm font-black text-orange-500 dark:text-[#10a37f] shadow-sm dark:shadow-none transition hover:-translate-y-0.5 hover:bg-[#fff5ec] dark:hover:bg-[#2f2f2f]"
            >
              Retry Quiz
            </button>
          )}
          {onClose && (
            <button
              onClick={onClose}
              className="rounded-2xl border border-gray-200 dark:border-[#424242] bg-white dark:bg-[#171717] px-5 py-2.5 text-sm font-black text-gray-600 dark:text-[#b4b4b4] shadow-sm dark:shadow-none transition hover:-translate-y-0.5 hover:bg-gray-50 dark:hover:bg-[#2f2f2f]"
            >
              Close
            </button>
          )}
          ...
          {isSaved === false && onSave && (
            <button
              onClick={onSave}
              className="rounded-2xl bg-[#6757ff] dark:bg-[#10a37f] px-5 py-2.5 text-sm font-black text-white shadow-sm dark:shadow-none transition hover:-translate-y-0.5 hover:shadow-md dark:hover:shadow-none"
            >
              ⭐ Save this attempt
            </button>
          )}
          ...
          {isSaved === true && (
            <div className="flex items-center gap-3 bg-gray-50 dark:bg-[#171717] rounded-2xl border border-gray-100 dark:border-[#424242] pl-4 pr-1.5 py-1.5 shadow-inner dark:shadow-none">
              <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">Saved ✓</span>
              {onUnsave && (
                <button
                  onClick={() => {
                  ...
                  className="rounded-xl border border-red-100 dark:border-red-900/50 bg-white dark:bg-[#2f2f2f] px-4 py-2 text-sm font-black text-red-500 dark:text-red-400 shadow-sm dark:shadow-none transition hover:bg-red-50 dark:hover:bg-red-900/30"
                >
                  Unsave
                </button>
              )}
        ...
        {quiz.questions.map((question, index) => {
          ...
          return (
            <div key={index} className="rounded-2xl border border-purple-50 dark:border-[#424242] bg-[#fffcf8] dark:bg-[#171717] p-5 shadow-sm dark:shadow-none space-y-3">
              ...
              <div className="grid gap-2 mt-3">
                {(() => {
                  ...
                  return optionsToRender.map((option, optIndex) => {
                    ...
                    let style = "bg-white dark:bg-[#2f2f2f] border-purple-100 dark:border-[#424242] text-[#15132b] dark:text-[#ececec]";
                    if (isCorrectAnswer) {
              ...
              <div className="mt-4 pt-2 border-t border-purple-50 dark:border-[#424242]">
                <button
                  type="button"
                  onClick={() => toggleExplanation(index)}
                  className="px-3 py-2 rounded-xl border border-purple-100 dark:border-[#424242] text-[11px] font-black text-[#6757ff] dark:text-[#10a37f] bg-white dark:bg-[#2f2f2f] transition hover:bg-purple-50 dark:hover:bg-[#424242]"
                >
